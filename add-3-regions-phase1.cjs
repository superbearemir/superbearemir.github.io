...
console.log("Starting Phase 1 index-based additions...");

// 1. Update Zw array
let idxZw = code.indexOf('id: "space_realm",');
if (idxZw === -1) idxZw = code.indexOf('id:"space_realm",');
let idxZwEnd = code.indexOf('}];', idxZw);
if (idxZwEnd !== -1) {
  const insertPos = idxZwEnd;
  const newRegionsStr = `,  {
    id: "volcano_cave",
  ...
  console.log("Zw array updated!");
} else {
  console.error("Zw end not found!");
}

// 2. Update STORY_CHAPTER_DATA
let idxStory = code.indexOf('STORY_CHAPTER_DATA = [');
let idxStoryEnd = code.indexOf('];// Story Cutscene Modal Component', idxStory);
if (idxStoryEnd !== -1) {
  const insertPos = idxStoryEnd;
  ...
  console.log("STORY_CHAPTER_DATA updated!");
} else {
  console.error("STORY_CHAPTER_DATA end not found!");
}

// 3. Update storyInfoMap in je
let idxInfoMap = code.indexOf('const storyInfoMap = {');
let idxInfoMapEnd = code.indexOf('};', idxInfoMap);
if (idxInfoMapEnd !== -1) {
  const insertPos = idxInfoMapEnd;
  const newInfoMapStr = `,
  ...
  console.log("storyInfoMap updated!");
} else {
  console.error("storyInfoMap end not found!");
}
...
if (code.includes(oldJeUnlock)) {
  ...
  console.log("je unlock sequence updated!");
} else {
...
if (code.includes(oldPortalSequence)) {
  ...
  console.log("spawnBossPortalForCurrentRegion updated!");
} else {
...
if (code.includes(oldModalIsUnlocked)) {
  ...
  console.log("StoryBookModal isUnlocked updated!");
} else {
...
if (code.includes(targetActiveStr)) {
  ...
  console.log("StoryBookModal isCurrentActive updated!");
} else {
  console.error("targetActiveStr not found!");
}

// 7. Update Quests array $w
let idxQuest = code.indexOf('id: "boss_space_warden"');
if (idxQuest === -1) idxQuest = code.indexOf('id:"boss_space_warden"');
let idxQuestEnd = code.indexOf('}];', idxQuest);
if (idxQuestEnd !== -1) {
  const insertPos = idxQuestEnd;
  const newQuestsStr = `,
  ...
  code = code.substring(0, insertPos) + newQuestsStr + code.substring(insertPos);
  console.log("Quests array $w updated!");
} else {
  console.error("Quests array $w end not found!");
}

fs.writeFileSync("src/game-bundle.js", code, "utf8");
console.log("Phase 1 completed with ZERO errors!");
